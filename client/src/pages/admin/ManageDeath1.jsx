...
const ManageDeath1 = () => {
  ...
  const updateStatus = async (id, newStatus, newReason = "") => {
    try {
      await axiosInstance.patch(`/certificateController/${id}`, {
        status: newStatus,
        reason: newReason,
      });
  ...
  const applyFilterAndSearch = sortedApps.filter((app) => {
    ...
    const matchesSearch =
      ...
      app.causeOfDeath.toLowerCase().includes(q) ||
      // app.eventPlace.toLowerCase().includes(q) ||
  ...
  ------------------------------------------------------------ */
  const exportCSV = () => {
    const headers = "ID,Name,DOD,CauseOfDeath,Place,Status\n";

    const rows = apps
      .map(
        (a) =>
          `${a.enrollment},${a.personName},${a.eventDate},${a.causeOfDeath},${a.eventPlace},${a.status}`
      )
      .join("\n");
    ...
    link.href = url;
    link.download = "death_applications.csv";
    link.click();
  };
        ...
        ------------------------------------------------------ */}
          <div className="flex flex-wrap justify-between mb-4">
            ...
            <div className="flex items-center bg-white border px-3 py-2 rounded-lg shadow-sm w-full md:w-1/3">
              ...
              <input
                type="text"
                placeholder="Search Name, cause, place, or ID"
            ...
            <div className="flex space-x-2 mt-3 md:mt-0">
              {["ALL", "PENDING", "COMPLETED", "CANCELED"].map((f) => (
                <button
                  key={f}
                  onClick={() => setFilter(f)}
                  className={`px-4 py-1 text-sm rounded-md ${filter === f ? "bg-blue-600 text-white" : "bg-white"
                    }`}
                >
                  {f}
                </button>
            ...
            <table className="min-w-full">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  {["id", "Name", "dod", "cause", "status"].map((field) => (
                    <th
                      key={field}
                ...
                {loading ? (
                  ...
                  currentApps.map((app) => (
                    <tr key={app.enrollment} className="hover:bg-gray-50">
                      ...
                      {/* Cause */}
                      <td className="px-6 py-4">{app.causeOfDeath}</td>

                      {/* Status Badge */}
                      <td className="px-6 py-4">
                        <span
                          className={`px-3 py-1 rounded-full text-xs font-semibold ${app.status === "COMPLETED"
